...
pub const IdentifierResolution = struct {
    ...
    pub fn resolve(self: *IdentifierResolution, program: c_ast.Program) c_ast.Program {
        ...
        const body = self.resolveBlock(new_program.function[0].body.?, &identifier_map) catch @panic("Failed to resolve block");
        new_program.function[0].body = body;
        return new_program;
    }
    ...
    fn resolveVariableDeclaration(self: *IdentifierResolution, declaration: c_ast.Declaration, identifier_map: *std.StringHashMap(MapEntry)) !c_ast.VariableDeclaration {
        var result = declaration.variable_declaration;
    ...
    fn resolveFunctionDeclaration(self: *IdentifierResolution, declaration: c_ast.Declaration, identifier_map: *std.StringHashMap(MapEntry)) !c_ast.FunctionDeclaration {
        var result = declaration.function_declaration;
        ...
        var new_map = self.cloneVariableMap(identifier_map);
        var new_params = try self.allocator.create([]const u8);
    ...
    fn resolveDeclaration(self: *IdentifierResolution, declaration: c_ast.Declaration, identifier_map: *std.StringHashMap(MapEntry)) !c_ast.Declaration {
        switch (declaration) {
            .variable_declaration => {
                return .{ .variable_declaration = try self.resolveVariableDeclaration(declaration, identifier_map) };
            },
            else => {
                return declaration;
            },
    ...
    fn resolveExp(self: *IdentifierResolution, expression: c_ast.Expression, identifier_map: *std.StringHashMap(MapEntry)) !c_ast.Expression {
        ...
        switch (result) {
            ...
            .function_call => {
                ...
                } else @panic("Undeclared function");
            },
        }
        return result;
    }
